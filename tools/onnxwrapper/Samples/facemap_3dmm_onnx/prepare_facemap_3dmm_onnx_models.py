...
# ---------------------------------------------------------------------
"""Prepare Facemap 3DMM ONNX model files.
"""
...
def _delete_with_retries(path: Path, retries: int = 10, delay_s: float = 0.25) -> None:
    ...
    # Final fallback: rename then delete
    if path.exists():
        try:
            ...
            # Give up silently; caller can decide whether to treat as error
            pass


def ensure_facemap_3dmm_onnx(
    ...
    *,
    ...
    """Download + extract facemap_3dmm ONNX zip into ./models-onnx and delete the zip."""
    ...
    ok = download_url(onnx_zip_url, onnx_zip_path, desc=f"Downloading {MODEL_NAME} ONNX zip ...")
    if not ok:
    ...
    if unzip:
        # Extract all files, then collect whatever .onnx files exist (keep original names)
        ...
        # Delete zip only AFTER ZipFile is closed
        _delete_with_retries(onnx_zip_path)
...
def main() -> None:
    paths = ensure_facemap_3dmm_onnx(unzip=True)
