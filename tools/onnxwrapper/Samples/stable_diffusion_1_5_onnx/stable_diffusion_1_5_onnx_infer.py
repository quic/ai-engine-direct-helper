...
def resolve_component_dir(model_root: Path, candidates):
    """Return the first existing subdirectory under model_root from a list of candidates."""
    for name in candidates:
        p = (model_root / name)
        if p.exists() and p.is_dir():
            return p
    raise FileNotFoundError(
        f"None of the expected component directories exist under {model_root}: {candidates}"
    )
...
class SD15OnnxPipeline:
    ...
    def _vae_decode(self, latents_nhwc: np.ndarray) -> np.ndarray:
        ...
        # Stable Diffusion latent scaling: many exports expect latents / 0.18215 before VAE decode
...
def main():
    ...
    parser.add_argument("--out", type=str, default="sd15_out.png")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Root directory of ONNX models (contains text_encoder/, unet/, vae_decoder/ (or vae/)). "
