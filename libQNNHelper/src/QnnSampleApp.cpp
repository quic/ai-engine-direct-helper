...
sample_app::StatusCode sample_app::QnnSampleApp::createDevice() {
  QnnDevice_Config_t** deviceConfigs{ nullptr };	// zw: For supporting BackendExtensions.
  ...
  if (nullptr != m_backendExtensions && m_backendExtensions->interface1()) {	// zw: For supporting BackendExtensions.
      ...
      while (!inputFileList[0].empty()) {
        ...
        if (iotensor::StatusCode::SUCCESS !=
            m_ioTensor.populateInputTensors(
                (uint32_t)graphIdx, inputFileList, inputs, graphInfo, m_inputDataType)) {
          returnStatus = StatusCode::FAILURE;
        }
        if (StatusCode::SUCCESS == returnStatus) {
          ...
          uint32_t configCount{ 0 };
          if (nullptr != m_backendExtensions && m_backendExtensions->interface1()) {
              if (!m_backendExtensions->interface1()->beforeExecute(graphInfo.graphName, &customGraphConfigs, &configCount)) {
          ...
          if (StatusCode::SUCCESS == returnStatus) {
            ...
            if (iotensor::StatusCode::SUCCESS !=
                m_ioTensor.writeOutputTensors((uint32_t)graphIdx,
                                              startIdx,
                                              graphInfo.graphName,
                                              outputs,
                                              graphInfo.numOutputTensors,
        ...
        if (StatusCode::SUCCESS == returnStatus) {
          ...
          if (StatusCode::SUCCESS == returnStatus) {
            ...
            for (size_t outputIdx = 0; outputIdx < graphInfo.numOutputTensors; outputIdx++) {
                ...
                uint8_t* buffer = reinterpret_cast<uint8_t*>(floatBuffer);
...
// BackendExtensions Setting for HTP
sample_app::StatusCode sample_app::QnnSampleApp::initializeBackendExtensions(BackendExtensionsConfigs backendExtensionsConfig) {
    std::unique_ptr<BackendExtensions> backendExtensions(
        new BackendExtensions(backendExtensionsConfig, m_backendLibraryHandle, PerfProfile::BURST));
...
sample_app::StatusCode sample_app::QnnSampleApp::boostPerformance() {
...
sample_app::StatusCode sample_app::QnnSampleApp::resetPerformance() {
    // Initialize the power config and select the voltage corner values for the performance setting.
    QnnHtpPerfInfrastructure_PowerConfig_t powerConfig;
    memset(&powerConfig, 0, sizeof(powerConfig));
