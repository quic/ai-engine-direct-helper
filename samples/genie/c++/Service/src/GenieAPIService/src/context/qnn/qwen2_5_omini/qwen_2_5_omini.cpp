...
IAudioEmbedding &QInterface::Qwen2_5OMINI::BuildAudioSamples()
{
    ...
    //auto out_frames_est = static_cast<uint64_t>(std::ceil(frames * src_ratio));
    ...
    //int rescaled_length = (out_frames_est - 1) / 160 + 1;
    //int input_length = (rescaled_length - 1) / 2 + 1;
    //audio_length_ = (input_length - 2) / 2 + 1;
    //clean();
    return *this;
}

IAudioEmbedding &GenieContext::QInterfaceImpl::IAudioEmbedding::PaddingAudioPrompt()
{
    std::string audio_list;
    audio_list.reserve(audio_length_ * strlen("<|AUDIO|>") + 1);
    for (int i = 0; i < audio_length_; ++i)
    {
        audio_list.append("<|AUDIO|>");
    }
    audio_list.back() = 0;
    std::string prompt_template(kPromptTemplate.size() + audio_list.size(), 0);
    sprintf(prompt_template.data(), kPromptTemplate.c_str(), "%s", audio_list.c_str());
