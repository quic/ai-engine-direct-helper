...
def model_download():
    if SOC_ID == "6490":
        print("Error: chipset 6490 is not supported for yolo26n_det yet.")
        sys.exit(1)
    
    desc = f"Downloading {MODEL_NAME} model... "
    fail = f"\nFailed to download {MODEL_NAME} model."
    hub_id = "iiabqzfd1m9frtafsmaigs0fhxls8znzeuo3iy24"
    ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME, model_path, desc=desc, fail=fail, hub_id=hub_id)
...
def Inference(input_image_path, output_image_path, show_image=True, confidence: float = 0.80):
    global image_buffer
