...
Logger::Logger(QnnLog_Callback_t callback, QnnLog_Level_t maxLevel, QnnLog_Error_t* status)
  ...
  if (!callback) {
    ...
    qnn::log::utils::logCreateLock();        // zw: We need share the lock between processes.
  }
}
...
// zw: Add for sync the time between processes.
uint64_t Logger::getTimediff() {
	return getTimestamp() - m_epoch;
}

uint64_t Logger::getEpoch() {
    return m_epoch;
}

void Logger::setEpoch(uint64_t epoch) {
    m_epoch = epoch;
}
...
// zw: Add for sync the time between processes.
uint64_t qnn::log::getTimediff() {
	return g_logger->getTimediff();
}

uint64_t qnn::log::getEpoch() {
    return g_logger->getEpoch();
}

void qnn::log::setEpoch(uint64_t epoch) {
    return g_logger->setEpoch(epoch);
}
